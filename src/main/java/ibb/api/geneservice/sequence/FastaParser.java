...
public final class FastaParser implements Iterator<FastaRecord> {
     ...
     */
    public static Stream<FastaRecord> parse(Path path) throws IOException {
        var stream = TextParser.parse(path);
        var lineIterator = stream.iterator();
        var fastaIterator = new FastaParser(lineIterator);
    ...
    private FastaParser(Iterator<String> lineIterator) {
